...
class GUI():
    ...
    def __init__(self):
        ...
        self.download_report_button = tk.Button(self.buttons_frame, text = 'download report',
                state = 'disabled', command = self.download_report, width = 32)
        self.download_video_button = tk.Button(self.buttons_frame, text = 'download video', 
                state = 'disabled', command = self.download_video, width = 32)
    ...
    def run(self):
        ...
        upload_file_button = tk.Button(self.buttons_frame, text = 'upload a video',
                command = self.upload_video, width = 32)
        upload_file_button.grid(row = 0, column = 0, pady = 10, padx = 10)


        start_algorithm_button = tk.Button(self.buttons_frame, text = 'start it', 
                command = self.start_algorithm, width = 32)
        start_algorithm_button.grid(row = 2, column = 0, pady = 10, padx = 10)

        play_uploaded_video_button = tk.Button(self.buttons_frame, text = 'play video', command = self.play_uploaded_video,
                width = 32)
        ...
        inside_player = tk.Button(self.buttons_frame, text = "player", command = self.inside_player, width = 32)
        ...
        menu_button = tk.Button(self.root, text = 'settings', width = 32, command = self.settings_window)
    ...
    def upload_video(self):
        ...
        if self.uploaded_file.endswith(('.mp4', '.mkv')):
            ...
            self.warning_window('Bad format', 'Wrong data format chosen! Select mp4 or mkv file')
            self.uploaded_file = ''
    ...
    def play_uploaded_video(self):
        if self.uploaded_file != '':
            ...
            self.warning_window('No video uploaded', 'No video uploaded! Upload one to play it')
    ...
    def warning_window(self, warning_title, warning_message):
        ...
        warning_label = tk.Label(self.top, text = warning_message, bg = 'grey', pady = 40, font = "Verdana 10 bold")
        warning_label.pack(anchor = 'center')
        exit_button = tk.Button(self.top, text = 'exit', command = self.top.destroy, width = 400, height = 35,
                bg = 'grey71')
    ...
    def settings_window(self):
        ...
        report_label = tk.Label(self.topSettings, text = 'Where to store report:', bg = 'grey', 
                pady = 25, font = "Verdana 10 bold")
        report_label.grid(row = 1, column = 0)
        report_button = tk.Button(self.topSettings, width = 35, text = 'Choose directory', command = self.choose_raport_location)
        report_button.grid(row = 1, column = 1, padx = 10, pady = 5)
        where_report_label = tk.Label(self.topSettings, text = 'Report name:', bg = 'grey', pady = 15,
                font="Verdana 10 bold")
        ...
        video_label = tk.Label(self.topSettings, text = 'Where to store video:', bg = 'grey',
                pady = 20, font = "Verdana 10 bold")
        video_label.grid(row = 2, column = 0)
        video_input = tk.Button(self.topSettings, width = 35, text = 'Choose directory',
                command = self.choose_video_location)
        video_input.grid(row = 2, column = 1, padx = 10, pady = 5)
        where_video_label = tk.Label(self.topSettings, text = 'Video name:', bg = 'grey',
        ...
        exit_button = tk.Button(self.topSettings, text = 'exit', 
                command = lambda: self.exit_settings(self.topSettings, where_video_input, where_report_input),
